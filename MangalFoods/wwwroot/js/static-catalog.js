(() => {
  const withBasePath = (target) => window.mangalFoods?.withBasePath
    ? window.mangalFoods.withBasePath(target)
    : `./${(target || "").replace(/^\/+/, "")}`;

  const dataPath = withBasePath("data/products.json");
  ...
  const toCardMarkup = (product) => {
    ...
    const primaryImage = Array.isArray(product.images) && product.images.length
      ? product.images[0]
      : withBasePath("images/products/placeholder-product.webp");
  ...
  const renderShopPage = (products) => {
    ...
    if (categoryInput) {
      categoryInput.value = state.category
        ? [...categoryInput.options].find((opt) => normalize(opt.value) === state.category)?.value || ""
        : "";
    }
  ...
  const renderProductPage = (products) => {
    ...
    const primaryImage = product.images?.[0] || withBasePath("images/products/placeholder-product.webp");
